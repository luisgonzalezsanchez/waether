...
class Postmark {
    ...
    function template_id($template_id){
        $this->_template_id = $template_id;
        return $this;
    }

    function template_model($template_model){
        $this->_template_model = $template_model;
        return $this;
    }
    ...
    function from($address, $name = null)
    {
        ...
        else
        {
            ...
            {
                $this->show_error('You have entered an invalid sender address.');
            }
        }
    }
    ...
    function to($address, $name = null)
    {
        ...
        else
        {
            ...
            {
                $this->show_error('You have entered an invalid recipient address.');
            }
        }
    }
    ...
    function reply_to($address, $name = null)
    {
        ...
        else
        {
            ...
            {
                $this->show_error('You have entered an invalid reply to address.');
            }
        }
    }
    ...
    */
    ...
    function _prepare_data_with_template()
    {
        $data = array();
        //$data['Subject'] = $this->_subject;
        ...
        $data['From'] = is_null($this->from_name) ? $this->from_address : "{$this->from_name} <{$this->from_address}>";
        $data['To'] = is_null($this->_to_name) ? $this->_to_address : "{$this->_to_name} <{$this->_to_address}>";
        $data['TemplateModel'] = is_null($this->_template_model)? "": $this->_template_model;
        $data['TemplateId'] = is_null($this->_template_id)? "":$this->_template_id;
        /*if (!is_null($this->_cc_address) && ($this->_cc_address != '')) {
            $data['Cc'] = is_null($this->_cc_name) ? $this->_cc_address : "{$this->_cc_name} <{$this->_cc_address}>";
        }
    ...
    function send($from_address = null, $from_name = null, $to_address = null, $to_name = null, $subject = null, $message_plain = null, $message_html = null, $display_unprocessable_messages = null)
    {
        ...
        if (is_null($this->api_key)) {
            $this->show_error("Postmark API key is not set!");
        }

        if (is_null($this->from_address)) {
            $this->show_error("From address is not set!");
        }

        if (is_null($this->_to_address)) {
            $this->show_error("To address is not set!");
        }

        if (is_null($this->_subject)) {
            $this->show_error("Subject is not set!");
        }

        if (is_null($this->_message_plain) && is_null($this->_message_html)) {
            $this->show_error("Please either set plain message, HTML message or both!");
        }
        ...
        if (curl_error($ch) != '') {
            //error_log('postmark: '.curl_error($ch));
            $this->show_error(curl_error($ch));
        }
        ...
        if ((intval($httpCode / 100) != 2) AND $this->display_unprocessable_messages) {
            ...
            $this->show_error('Error while mailing. Postmark returned HTTP code ' . $httpCode . ' with message "'.$message.'"');
        }
    }

    function send_with_template($from_address = null, $from_name = null, $to_address = null, $to_name = null, $subject = null, $message_plain = null, $message_html = null, $display_unprocessable_messages = null)
    {
        ...
        if (is_null($this->_template_id)) {
            $this->show_error("Template id is not set!");
        }

        if (is_null($this->_template_model)) {
            $this->show_error("Template model is not set!");
        }

        $encoded_data = json_encode($this->_prepare_data_with_template());
        //echo $encoded_data;
        ...
        //error_log('POSTMARK JSON: ' . $encoded_data . "\nHeaders: \n\t" . implode("\n\t", $headers) . "\nReturn:\n$return");
        ...
        echo json_encode($http_code);
        echo json_encode($curl_error);      
        curl_close($ch);
